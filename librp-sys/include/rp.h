...
typedef enum {
  ...
  RP_GEN_MODE_BURST, //!< Signal is generated N times, wher N is defined with
                     //!< rp_GenBurstCount method
  RP_GEN_MODE_STREAM //!< User can continuously write data to buffer
} rp_gen_mode_t;

typedef enum {
  RP_GEN_SWEEP_DIR_NORMAL, //!< Generate sweep signal from start frequency to
                           //!< end frequency
  RP_GEN_SWEEP_DIR_UP_DOWN //!< Generate sweep signal from start frequency to
...
typedef enum {
  RP_SMP_125M =
      1, //!< Sample rate 125Msps; Buffer time length 131us; Decimation 1
  RP_SMP_62_500M =
      2, //!< Sample rate 62.5Msps; Buffer time length 262us; Decimation 2
  RP_SMP_31_250M =
      4, //!< Sample rate 31.25Msps; Buffer time length 524us; Decimation 4
  RP_SMP_15_625M =
      8, //!< Sample rate 15.625Msps; Buffer time length 1.048ms; Decimation 8
  RP_SMP_7_812M =
      16, //!< Sample rate 7.8125Msps; Buffer time length 2.096ms; Decimation 16
  RP_SMP_3_906M =
      32, //!< Sample rate 3.906Msps; Buffer time length 4.192ms; Decimation 32
  RP_SMP_1_953M =
      64, //!< Sample rate 1.953Msps; Buffer time length 8.388ms; Decimation 64
  RP_SMP_976_562K =
      128, //!< Sample rate 976ksps; Buffer time length 16.768ms; Decimation 128
  RP_SMP_448_281K =
      256, //!< Sample rate 488ksps; Buffer time length 33.798ms; Decimation 256
  RP_SMP_244_140K =
      512, //!< Sample rate 244ksps; Buffer time length 67.07ms; Decimation 512
  RP_SMP_122_070K = 1024, //!< Sample rate 122.070ksps; Buffer time length
                          //!< 134.2ms; Decimation 1024
  RP_SMP_61_035K = 2048,  //!< Sample rate 61.035ksps; Buffer time length
                          //!< 268.288ms; Decimation 2048
  RP_SMP_30_517K = 4096,  //!< Sample rate 30.517ksps; Buffer time length
                          //!< 536.5ms; Decimation 4096
  RP_SMP_15_258K = 8192, //!< Sample rate 15.258ksps; Buffer time length 1.073s;
                         //!< Decimation 8192
  RP_SMP_7_629K = 16384, //!< Sample rate 7.629ksps; Buffer time length 2.146s;
                         //!< Decimation 16384
  RP_SMP_3_814K = 32768, //!< Sample rate 3.814ksps; Buffer time length 4.292s;
                         //!< Decimation 32768
  RP_SMP_1_907K = 65536  //!< Sample rate 1.907ksps; Buffer time length 8.589s;
                         //!< Decimation 65536
...
typedef enum {
  ...
  RP_DEC_512 =
      512, //!< Sample rate 244ksps; Buffer time length 67.07ms; Decimation 512
  RP_DEC_1024 = 1024, //!< Sample rate 122.070ksps; Buffer time length 134.2ms;
                      //!< Decimation 1024
  RP_DEC_2048 = 2048, //!< Sample rate 61.035ksps; Buffer time length 268.288ms;
                      //!< Decimation 2048
  RP_DEC_4096 = 4096, //!< Sample rate 30.517ksps; Buffer time length 536.5ms;
                      //!< Decimation 4096
  RP_DEC_8192 = 8192, //!< Sample rate 15.258ksps; Buffer time length 1.073s;
                      //!< Decimation 8192
  RP_DEC_16384 = 16384, //!< Sample rate 7.629ksps; Buffer time length 2.146s;
                        //!< Decimation 16384
  RP_DEC_32768 = 32768, //!< Sample rate 3.814ksps; Buffer time length 4.292s;
                        //!< Decimation 32768
  RP_DEC_65536 = 65536  //!< Sample rate 1.907ksps; Buffer time length 8.589s;
                        //!< Decimation 65536
} rp_acq_decimation_t;
...
typedef enum {
  ...
  RP_TRIG_SRC_EXT_PE, //!< Trigger set to external trigger positive edge (DIO0_P
                      //!< pin)
  RP_TRIG_SRC_EXT_NE, //!< Trigger set to external trigger negative edge (DIO0_P
                      //!< pin)
  RP_TRIG_SRC_AWG_PE, //!< Trigger set to arbitrary wave generator application
                      //!< positive edge
  RP_TRIG_SRC_AWG_NE  //!< Trigger set to arbitrary wave generator application
                      //!< negative edge
} rp_acq_trig_src_t;
...
typedef struct {
  uint32_t
      fe_ch1_fs_g_hi; //!< High gain front end full scale voltage, channel A
  uint32_t
      fe_ch2_fs_g_hi; //!< High gain front end full scale voltage, channel B
  ...
  uint32_t low_filter_aa_ch1; //!< Filter equalization coefficients AA for Low
                              //!< mode, channel A
  uint32_t low_filter_bb_ch1; //!< Filter equalization coefficients BB for Low
                              //!< mode, channel A
  uint32_t low_filter_pp_ch1; //!< Filter equalization coefficients PP for Low
                              //!< mode, channel A
  uint32_t low_filter_kk_ch1; //!< Filter equalization coefficients KK for Low
                              //!< mode, channel A
  uint32_t low_filter_aa_ch2; //!< Filter equalization coefficients AA for Low
                              //!< mode, channel B
  uint32_t low_filter_bb_ch2; //!< Filter equalization coefficients BB for Low
                              //!< mode, channel B
  uint32_t low_filter_pp_ch2; //!< Filter equalization coefficients PP for Low
                              //!< mode, channel B
  uint32_t low_filter_kk_ch2; //!< Filter equalization coefficients KK for Low
                              //!< mode, channel B
  uint32_t hi_filter_aa_ch1;  //!< Filter equalization coefficients AA for High
                              //!< mode, channel A
  uint32_t hi_filter_bb_ch1;  //!< Filter equalization coefficients BB for High
                              //!< mode, channel A
  uint32_t hi_filter_pp_ch1;  //!< Filter equalization coefficients PP for High
                              //!< mode, channel A
  uint32_t hi_filter_kk_ch1;  //!< Filter equalization coefficients KK for High
                              //!< mode, channel A
  uint32_t hi_filter_aa_ch2;  //!< Filter equalization coefficients AA for High
                              //!< mode, channel B
  uint32_t hi_filter_bb_ch2;  //!< Filter equalization coefficients BB for High
                              //!< mode, channel B
  uint32_t hi_filter_pp_ch2;  //!< Filter equalization coefficients PP for High
                              //!< mode, channel B
  uint32_t hi_filter_kk_ch2;  //!< Filter equalization coefficients KK for High
                              //!< mode, channel B
...
/**
 * Initializes the library. It must be called first, before any other library
 * method.
...
/**
 * Releases the library resources. It must be called last, after library is not
...
/**
 * Sets digital input output pin direction. LED pins are already automatically
...
/**
 * Indicates whether the ADC buffer was full of data. The length of the buffer
...
/**
 * Convert factor to decimation used at acquiring signal. There is only a get of
 * pre-defined decimation values which can be specified. See the
 * #rp_acq_decimation_t enum values.
...
/**
 * Sets the sampling rate for acquiring signal. There is only a set of
...
/**
 * Gets the sampling rate for acquiring signal. There is only a set of
...
/**
 * Returns the trigger state. Either it is waiting for a trigger to happen, or
...
/**
 * Sets the trigger threshold value in volts. Makes the trigger when ADC value
 * crosses this value.
...
/**
 * Sets the acquire gain state. The gain should be set to the same value as it
...
/**
 * Returns the currently set acquire gain state in the library. It may not be
...
/**
 * Returns the currently set acquire gain in the library. It may not be set to
...
/**
 * Starts the acquire. Signals coming from the input channels are acquired and
 * written into memory.
 * @return If the function is successful, the return value is RP_OK.
 * If the function is unsuccessful, the return value is any of RP_E* values that
 * indicate an error.
...
/**
 * Normalizes the ADC buffer position. Returns the modulo operation of ADC
 * buffer size...
...
/**
 * Returns the ADC buffer in Volt units from specified position and desired
...
/**
 * Returns the ADC buffer in Volt units from the oldest sample to the newest
...
/**
 * Sets channel signal frequency in fpga without reset generator and rebuild
 * signal.
 * @param channel Channel A or B for witch we want to set frequency.
 * @param frequency Frequency of the generated signal in Hz.
 * @return If the function is successful, the return value is RP_OK.
...
/**
 * Sets number of burst repetitions. This determines how many bursts will be
 * generated.
...
/**
 * Sets the time/period of one burst in micro seconds. Period must be equal or
...
float rp_CmnCnvCntToV(uint32_t field_len, uint32_t cnts, float adc_max_v,
                      uint32_t calibScale, int calib_dc_off, float user_dc_off);

const volatile uint32_t *rp_jmd_AcqGetRawBuffer(rp_channel_t channel);
